    ...
    if(requestData == nil) {
        ...
        if (![url containsString:@"/login"]) {
            if ([[UserData shareInstance] getToken]) {
                ...
                NSString* description = [NSString stringWithFormat:@"Invalid token: %ld - %@", (long)self.httpCode, [NSHTTPURLResponse localizedStringForStatusCode:self.httpCode]];
                [m_pParams put:KeyResponseResult value:@(ResultErrorInvalidToken)];
                [m_pParams put:KeyResponseError value:description];
                
                if (self.delegate != nil && [self.delegate respondsToSelector:@selector(completed:data:)]) {
                    [self.delegate completed:self data:m_pParams];
                }
                return;
            }
