...
@property NSMutableArray *separatos;
...
-(void) viewDidAppear:(BOOL)animated{
    ...
    if(self.shouldEqualWidth == YES){
        NSInteger numberOfTab = MAX(1, self.tabCount);
        _tabWidth = (CGRectGetWidth(self.tabsView.frame) - ((numberOfTab-1) * 2.0)) / numberOfTab;
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex
{
    ...
    UIView *lastTabView = [self tabViewAtIndex:self.activeTabIndex];
    
    UILabel *lastLabel = (UILabel *)[[lastTabView subviews] objectAtIndex:0];
    if (_unSelectedTextColor) {
        lastLabel.textColor = _unSelectedTextColor;
    }else{
        lastLabel.textColor = RGB(108.0, 108.0, 108.0);
    }
    ...
    UILabel *label = (UILabel *)[[tabView subviews] objectAtIndex:0];
    if(_selectedTextColor){
        label.textColor = _selectedTextColor;
    }else{
        label.textColor = RGB(14, 114, 194);
    }
...
- (void)setNeedsReloadOptions
{
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        ...
        if((i > 0 && self.shouldShowdividerAllItem == YES) ||
           (i == 1 && self.shouldShowdividerAtFirstItem == YES)){
            //Add separator
            
            CGRect dividerFrame = CGRectMake(contentSizeWidth,
                                             self.tabHeight / 4.0,
                                             1.0,
                                             (self.tabHeight / 2.0));
            UIView* subview = [self.tabsView viewWithTag:i];
            if(subview != nil){
                subview.frame = dividerFrame;
                contentSizeWidth += CGRectGetWidth(subview.frame);
            }
        }
...
- (void)defaultSettings
{
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
        ...
        self.indicatorHeight = 2.0*kIndicatorHeight/3.0;
    self.tabOffset = kTabOffset;
    self.tabWidth = kTabWidth;
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ) {
        self.tabHeight = kTabHeight* 1.5;
    }else
        self.tabHeight = kTabHeight * 1.2;
    self.padding = 0.0;
    ...
    self.shouldShowdividerAtFirstItem = NO;
    self.shouldEqualWidth = NO;
    self.shouldShowdividerAllItem = NO;
    self.shouldShowMoreItem = nil;
    self.shouldHideTabFromIndex = -1;
}
...
//    if( self.tabCount == 1){
        //disable interaction
        ((UIScrollView *) self.pageViewController.view.subviews[0]).bounces = NO;
    ((UIScrollView *) self.pageViewController.view.subviews[0]).scrollEnabled = NO;
  ...
  //  }
    ...
    if (!self.tabsView) {
        ...
        if(self.shouldShowMoreItem != nil){
            //Add more indicator
            CGFloat width = CGRectGetWidth(self.view.frame) - (self.tabHeight / 2);
            self.tabsView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.0, 0.0, width, self.tabHeight)];
            [self.view insertSubview:self.tabsView atIndex:0];

            
            UIButton* moreItem = [UIButton buttonWithType:UIButtonTypeCustom];
            moreItem.frame = CGRectMake(width, 0, (self.tabHeight / 2), self.tabHeight);
            moreItem.backgroundColor = self.tabsViewBackgroundColor;
            [moreItem handleControlEvent:UIControlEventTouchUpInside withBlock:self.shouldShowMoreItem];
            
            [moreItem setImage:[UIImage imageNamed:@"down_arrow"] forState:UIControlStateNormal];
            [moreItem setImage:[UIImage imageNamed:@"down_arrow"] forState:UIControlStateHighlighted];
            moreItem.contentMode = UIViewContentModeScaleAspectFit;
            [self.view addSubview:moreItem];
            
            [self.tabsView setTranslatesAutoresizingMaskIntoConstraints:NO];
            [moreItem setTranslatesAutoresizingMaskIntoConstraints:NO];

            
            NSLayoutConstraint *tabViewLeading =[NSLayoutConstraint
                                                 constraintWithItem:self.tabsView
                                                 attribute:NSLayoutAttributeLeading
                                                 relatedBy:NSLayoutRelationEqual
                                                 toItem:self.tabsView.superview
                                                 attribute:NSLayoutAttributeLeading
                                                 multiplier:1.0
                                                 constant:0];
            
            NSLayoutConstraint *tabViewTrailing =[NSLayoutConstraint
                                                  constraintWithItem:self.tabsView
                                                  ...
                                                  multiplier:1.0
                                                  constant:0];
            NSLayoutConstraint *tabViewHeight =[NSLayoutConstraint
                                                constraintWithItem:self.tabsView
            ...
            NSLayoutConstraint *tabViewTop =[NSLayoutConstraint
                                             constraintWithItem:self.tabsView
            ...
            [self.view addConstraint:tabViewHeight];
            [self.view addConstraint:tabViewLeading];
            [self.view addConstraint:tabViewTrailing];
            [self.view addConstraint:tabViewTop];
            
            
            NSLayoutConstraint *moreTop =[NSLayoutConstraint
                                          constraintWithItem:moreItem
                                          attribute:NSLayoutAttributeTop
                                          relatedBy:NSLayoutRelationEqual
                                          toItem:self.tabsView
                                          attribute:NSLayoutAttributeTop
                                          multiplier:1.0
                                          constant:0];
            NSLayoutConstraint *moreHeight =[NSLayoutConstraint
                                             constraintWithItem:moreItem
                                             ...
                                             multiplier:1.0
                                             constant:0];
            NSLayoutConstraint *moreRatio =[NSLayoutConstraint
                                            constraintWithItem:moreItem
            ...
            NSLayoutConstraint *moreTrailing =[NSLayoutConstraint
                                               constraintWithItem:moreItem
                                               attribute:NSLayoutAttributeTrailing
                                               relatedBy:NSLayoutRelationEqual
                                               toItem:self.view
            ...
            [self.view addConstraint:moreTop];
            [self.view addConstraint:moreHeight];
            [self.view addConstraint:moreRatio];
            [self.view addConstraint:moreTrailing];
        }else{
        ...
        if((i > 0 && self.shouldShowdividerAllItem == YES) ||
           (i == 1 && self.shouldShowdividerAtFirstItem == YES)){
            ...
            CGRect dividerFrame = CGRectMake(contentSizeWidth,
                                             ...
                                             1.0,
                                             (self.tabHeight / 2.0));
            UIView* subview = [[UIView alloc] initWithFrame:dividerFrame];
            subview.backgroundColor = [UIColor lightGrayColor];
            subview.tag = i;
            //subview.translatesAutoresizingMaskIntoConstraints = NO;
            
            [self.tabsView addSubview:subview];
            contentSizeWidth += CGRectGetWidth(subview.frame);
        }
...
- (TabView *)tabViewAtIndex:(NSUInteger)index
{
    ...
    if ([self.tabs[index] isEqual:[NSNull null]]) {
        ...
        if(self.shouldEqualWidth == YES){
            NSInteger numberOfTab = MAX(1, self.tabCount);
            tabWidth = (CGRectGetWidth(self.tabsView.frame) - ((numberOfTab-1) * 2.0)) / numberOfTab;
            itemWidth = tabWidth;
        }else{
            ...
            tabWidth = itemWidth + 24;
        }
        tabViewContent.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
        
        
        if(self.shouldHideTabFromIndex > -1 && index >= self.shouldHideTabFromIndex){
            tabWidth = 0.0;
        }
        // Create TabView and subview the content
        TabView *tabView = [[TabView alloc] initWithFrame:CGRectMake(0.0, 0.0, tabWidth, self.tabHeight)];
        
        tabViewContent.frame = CGRectMake(0.0f, 0.0f, itemWidth, self.tabHeight);
